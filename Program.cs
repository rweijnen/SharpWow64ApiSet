...
namespace Wow64Test
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool bResult;
            bResult = WinApi.IsWow64Process2(WinApi.GetCurrentProcess(), out processMachine, out nativeMachine);
            if (bResult)
            {
                Console.WriteLine("ProcessMachine: {0}, NativeMachine: {1}", WinApi.MachineTypeToStr(processMachine), WinApi.MachineTypeToStr(nativeMachine));
            }
            else
            {
                int e = Marshal.GetLastWin32Error();
                string s = new Win32Exception(e).Message;
                Console.WriteLine("IsWow64Process2 failed with error {0} ({1})", e, s);
            }

            bool bMachineIsSupported;
            UInt32 hr = WinApi.IsWow64GuestMachineSupported(WinApi.IMAGE_FILE_MACHINE_ARM, out bMachineIsSupported);
            if (hr == WinApi.S_OK)
            {
                Console.WriteLine("IMAGE_FILE_MACHINE_ARM: {0}", bMachineIsSupported);
            }
            else
            {
                int e = Marshal.GetLastWin32Error();
                string s = new Win32Exception(e).Message;
                Console.WriteLine("IsWow64GuestMachineSupported failed with error {0} ({1}) and HRESULT: 0x{0:8}", e, s, hr);
